...
def plot_distribution(y1, y2, label1, label2, colors=['b', 'g'], cut=None):
    """
    plot the distribution 2 variables on the same axis

    :param y1: 1st variable
    :param y2: 2md variable
    :param label1: label of first variable
    :param label2: label of 2nd variable
    :param colors: colors to be used for the plots
    :param cut: option to mark a cut/threshold as a dashed vertical line(if any)
...
def calc_priodic_penalty(data_ob, object_df):
    """
    calculate the penalty for lighcurve (TODO: remove dependency on data_ob with an object of LC)
    :param data_ob: data object with all the data
    :param object_df: data of the current object whose penalty is to be calculated
    :return: returns the penalty for current object
    """
...
def PLAsTiCC_transient_filter(data_ob, cut=8):
    """
    function to filter transients for LSST data. The function adds the result to the meta_data of data object

    :param data_ob: data objects with the light-curves whose penalty is to be calculated
    :param cut: cut on the penalty to accept/reject events based on the penalty
    :return: array with the results of filter process
    """
...
def transient_filter_load_saved(data_ob, path="filter_result.npy"):
    """
    load filter results if already saved

    :param data_ob: data object to which filter is to be added
    :param path: path where filter results
    :return:
    """
